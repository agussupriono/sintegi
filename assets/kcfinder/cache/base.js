...
_.initOpeners = function() {
    ...
    try {
        ...
        // TinyMCE 3
        if (_opener.name == "tinymce") {
            if (typeof tinyMCEPopup == "undefined")
                _opener.name = null;
            else
                _opener.callBack = true;

        // TinyMCE 4
        } else if (_opener.name == "tinymce4")
            _opener.callBack = true;

        // CKEditor
        else if (_opener.name == "ckeditor") {
            if (window.parent && window.parent.CKEDITOR)
                _opener.CKEditor.object = window.parent.CKEDITOR;
            else if (window.opener && window.opener.CKEDITOR) {
                _opener.CKEditor.object = window.opener.CKEDITOR;
                _opener.callBack = true;
            } else
                _opener.CKEditor = null;

        // FCKeditor
        } else if ((!_opener.name || (_opener.name == "fckeditor")) && window.opener && window.opener.SetUrl) {
            _opener.name = "fckeditor";
            _opener.callBack = true;
        }
        ...
        if (!_opener.callBack) {
            if ((window.opener && window.opener.KCFinder && window.opener.KCFinder.callBack) ||
                ...
                _opener.callBack = window.opener
            ...
            if ((
                ...
                _opener.callBackMultiple = window.opener
                    ? window.opener.KCFinder.callBackMultiple
                    : window.parent.KCFinder.callBackMultiple;
        }
...
_.initToolbar = function() {
    ...
    $('#toolbar a[href="kcact:about"]').click(function() {
        var html = '<div class="box about">' +
            '<div class="head"><a href="http://kcfinder.sunhater.com" target="_blank">KCFinder</a> ' + _version + '</div>';
        if (_support_check4Update)
    ...
    setTimeout(function() {
        ...
        $.each(_.files, function(i, file) {
            ...
            // Thumbnails
                ...
                if (file.thumb)
                    icon = _.getURL('thumb') + "&file=" + encodeURIComponent(file.name) + "&dir=" + encodeURIComponent(_dir) + "&stamp=" + stamp;
                else if (file.smallThumb) {
                    icon = _uploadURL + "/" + _dir + "/" + encodeURIComponent(file.name);
                    icon = $.$.escapeDirs(icon).replace(/\'/g, "%27");
                } else {
                    icon = file.bigIcon ? $.$.getFileExtension(file.name) : ".";
                    if (!icon.length) icon = ".";
                    icon = "themes/" + _theme + "/img/files/big/" + icon + ".png";
                }
...
_.returnFile = function(file) {
    ...
    var button, win, fileURL = file.substr
        ? file : _uploadURL + "/" + _dir + "/" + file.data('name');
    fileURL = $.$.escapeDirs(fileURL);

    if (_opener.name == "ckeditor") {
        _opener.CKEditor.object.tools.callFunction(_opener.CKEditor.funcNum, fileURL, "");
        window.close();

    } else if (_opener.name == "fckeditor") {
        window.opener.SetUrl(fileURL) ;
        window.close() ;

    } else if (_opener.name == "tinymce") {
        ...
        $(win.document).find('#' + _opener.TinyMCE.field).val(fileURL);
        win.tinyMCE.activeEditor.windowManager.close();

    } else if (_opener.callBack) {

        if (window.opener && window.opener.KCFinder) {
            _opener.callBack(fileURL);
            window.close();
        }
    ...
    } else if (_opener.callBackMultiple) {
        if (window.opener && window.opener.KCFinder) {
            _opener.callBackMultiple([fileURL]);
            window.close();
        }
...
_.returnFiles = function(files) {
    if (_opener.callBackMultiple && files.length) {
        var rfiles = [];
        $.each(files, function(i, file) {
            rfiles[i] = _uploadURL + "/" + _dir + "/" + $(file).data('name');
            rfiles[i] = $.$.escapeDirs(rfiles[i]);
        });
        _opener.callBackMultiple(rfiles);
...
_.returnThumbnails = function(files) {
    if (_opener.callBackMultiple) {
        ...
        $.each(files, function(i, file) {
            if ($(file).data('thumb')) {
                rfiles[j] = _thumbsURL + "/" + _dir + "/" + $(file).data('name');
...
_.expandDir = function(dir) {
    ...
    if (dir.children('.brace').hasClass('opened')) {
        dir.parent().children('.folders').hide(500, function() {
            if (path == _dir.substr(0, path.length))
                _.changeDir(dir);
        });
...
_.viewImage = function(data) {
    ...
    showImage = function(data) {
        ...
        var url = $.$.escapeDirs(_uploadURL + "/" + _dir + "/" + data.name) + "?ts=" + ts,
            img = new Image(),
            i = $(img),
            w = $(window),
            d = $(document);
    ...
    function processUploadQueue() {
        ...
        if (uploadQueue && uploadQueue.length) {
            ...
            var loop = setInterval(function() {
                ...
                uploadQueue = [];
                clearInterval(loop);
                if (currentFile.thisTargetDir == _dir)
                    _.refresh();
                if (errors.length) {
                    errors = errors.join("\n");
                    if (errors.replace(/^\s+/g, "").replace(/\s+$/g, "").length)
...
_.getURL = function(act) {
    var url = "browse.php?type=" + encodeURIComponent(_type) + "&lng=" + encodeURIComponent(_lang);
    if (_opener.name)
        url += "&opener=" + encodeURIComponent(_opener.name);
    if (act)
        url += "&act=" + encodeURIComponent(act);
    if (_cms)
        url += "&cms=" + encodeURIComponent(_cms);
    return url;
};
