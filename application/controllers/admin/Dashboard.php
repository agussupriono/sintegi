...
class Dashboard extends CI_Controller {
    ...
    public function config() {
        $site = $this->mConfig->list_config();
        $themes = $this->mThemes->listThemes();
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $data = array('config_id' => $i->post('config_id'),
                ...
                'theme' => $i->post('theme'),
                'color' => $i->post('color'),
                'color_2' => $i->post('color_2'),
                'color_3' => $i->post('color_3')
            );
        ...
        if ($v->run()) {
            ...
            if (!$this->upload->do_upload('logo')) {
                ...
                if ($this->upload->do_upload('logo1')) {
                    $upload_data['uploads1'] = $this->upload->data();
                    // Image Editor
                    $config['image_library'] = 'gd2';
                    $config['source_image'] = './assets/upload/image/' . $upload_data['uploads1']['file_name'];
                    $config['new_image'] = './assets/upload/image/thumbs/';
                    $config['create_thumb'] = TRUE;
                    $config['maintain_ratio'] = TRUE;
                    $config['width'] = 246; // Pixel
                ...
                if ($this->upload->do_upload('logo2')) {
                    $upload_data['uploads2'] = $this->upload->data();
                    // Image Editor
                    $config['image_library'] = 'gd2';
                    $config['source_image'] = './assets/upload/image/' . $upload_data['uploads2']['file_name'];
                    $config['new_image'] = './assets/upload/image/thumbs/';
                    $config['create_thumb'] = TRUE;
                    $config['maintain_ratio'] = TRUE;
                    $config['width'] = 246; // Pixel
                ...
                $data = array('config_id' => $i->post('config_id'),
                    ...
                    'logo1' => $upload_data['uploads1']['file_name'],
                    'logo2' => $upload_data['uploads2']['file_name']
                );
                $this->mConfig->edit_config($data);
