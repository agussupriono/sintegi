...
class Blog extends CI_Controller {
    ...
    public function index() {
        ...
        $data = array('title' => 'Blog - ' . $site['nameweb'],
            ...
            'isi' => 'front/'.$site['theme'].'/blog/list');
    ...
    public function cari() {
        ...
        $data['blog'] = $this->mBlogs->cariBlog($pagination['per_page'], $this->uri->segment(4, 0), $data['ringkasan'], $data['isi'] = 'front/'.$site['theme'].'/blog/cari', $data['title'] = 'Hasil Pencarian - ' . $data['ringkasan'], $data['site'] = $site, $data['categories'] = $categories, $data['lastBlogs'] = $lastBlogs, $data['pagin'] = $this->pagination->create_links()
        );

        $this->load->vars($data);
        $this->load->view('front/'.$site['theme'].'/layout/wrapper');
    }
    ...
    public function detil($slugBlog) {
        ...
        if($blog){
            ...
            $data = array('title' => 'Not Found' . ' - ' . $site['nameweb'],
                'site' => $site,
                'menus' => $menus,
                'isi' => 'front/'.$site['theme'].'/layout/error_404');
            $this->load->view('front/'.$site['theme'].'/layout/wrapper', $data);
        }
    }
    ...
    public function kategori($slugBlog) {
        ...
        $data = array('title' => $detailCategory['category_name'] . ' - ' . $site['nameweb'],
            ...
            'menus' => $menus,
            'pagin' => $this->pagination->create_links(),
            'isi' => 'front/'.$site['theme'].'/blog/category');
        $this->load->view('front/'.$site['theme'].'/layout/wrapper', $data);
    }
