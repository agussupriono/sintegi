<?php
    ...
    class Products_model extends CI_Model {
        ...
        // Listing Products Publish For Home Section Three
        public function listProductsPubForSectionThree() {
            $this->db->select('*');
            $this->db->from('products');
            $this->db->where(array('status' => 'publish', 'flag_home' => '1'));                           
            $this->db->join('admins','admins.admin_id = products.user_id','LEFT');                        
            $this->db->order_by('product_id','ASC');
            $query = $this->db->get();
            return $query->result_array();
        ...
        // Listing Products Publish
        public function listProductsPubPosition($pos) {
            $this->db->select('*');
            $this->db->from('products');
            $this->db->where(array('status' => 'publish', 'position' => $pos));                           
            $this->db->join('admins','admins.admin_id = products.user_id','LEFT');                        
            $this->db->order_by('product_id','ASC');
            $query = $this->db->get();
            return $query->result_array();
        ...
        // Listing Products Publish
        public function listProductsPubPositionSeq($pos) {
            $this->db->select('*');
            $this->db->from('products');
            $this->db->where(array('status' => 'publish', 'position' => $pos));                           
            $this->db->join('admins','admins.admin_id = products.user_id','LEFT');                        
            $this->db->order_by('products.seq','ASC');
            $query = $this->db->get();
            return $query->result_array();
        } 
