<?php
    ...
    class Galleries_model extends CI_Model {
        ...
        public function listGalleriesPubPosition($pos) {
            $this->db->select('*');
            $this->db->from('galleries');
        ...
        // Listing Galleries Slider 
        public function listGalleriesHomeBackground() {
            $this->db->select('*');
            $this->db->from('galleries');
            $this->db->where(array('status' => 'publish','position' => 'background'));                        
            $this->db->order_by('gallery_id','ASC');                   
            $this->db->limit(1);
            $query = $this->db->get();
            return $query->row_array();
        ...
        // Listing Galleries Slider 
        public function listGalleriesTeamCertificate() {
            $this->db->select('*');
            $this->db->from('galleries');
            $this->db->where(array('status' => 'publish','position' => 'teamcert'));                        
            $this->db->order_by('seq','ASC');                   
            $query = $this->db->get();
            return $query->result_array();
        }
